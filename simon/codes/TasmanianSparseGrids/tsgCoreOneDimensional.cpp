...
CustomTabulated::CustomTabulated( const char* filename ) : num_levels(0), num_nodes(0), precision(0), offsets(0), nodes(0), weights(0), description(0)
{
        std::ifstream ifs; ifs.open( filename );
        if ( !read( ifs ) ){
                cerr << "ERROR: could not read the custom rule! Check the file format." << endl;
                exit(1);
        }
        ifs.close();
}
...
bool CustomTabulated::read( std::ifstream &ifs ){
        ...
        std::string T;
        ifs >> T;
        if ( !(T.compare( "description:" ) == 0) ){ cerr << "ERROR: wrong file format of custom tables on line 1" << endl; ifs.close(); return false; }
        description = new std::string;
        getline( ifs, *description );

        ifs >> T;
        if ( !(T.compare( "levels:" ) == 0) ){ cerr << "ERROR: wrong file format of custom tables on line 2" << endl; ifs.close(); return false; }
        ifs >> num_levels;
...
int CustomTabulated::getNumPoints( int level ) const{  return num_nodes[level];  }
int CustomTabulated::getIExact( int level ) const{  return num_nodes[level] -1;  }
int CustomTabulated::getQExact( int level ) const{  return precision[level];  }
...
int OneDimensionalMeta::getNumPoints( int level, TypeOneDRule rule ) const{
        ...
        switch (rule){
                ...
                case rule_customtabulated:    if ( level < custom->getNumLevels()-1 ){ return custom->getNumPoints( level ); }else{  cerr << "ERROR: requested custom rule with level " << level << " the tabulated rules end at " << custom->getNumLevels()-1 << endl; exit(1);  }
...
int OneDimensionalMeta::getIExact( int level, TypeOneDRule rule ) const{
        switch (rule){
                ...
                case rule_customtabulated:    if ( level < custom->getNumLevels()-1 ){ return custom->getIExact( level ); }else{  cerr << "ERROR: requested custom rule with level " << level << " the tabulated rules end at " << custom->getNumLevels()-1 << endl; exit(1);  }
                default:
                        return level;
        }
}
int OneDimensionalMeta::getQExact( int level, TypeOneDRule rule ) const{
        switch (rule){
                ...
                case rule_rlejadouble4:       return getNumPoints(level,rule_rlejadouble4)-1;
                case rule_fejer2:             return ( ( 1 << (level+1) ) - 1 );
                case rule_customtabulated:    if ( level < custom->getNumLevels()-1 ){ return custom->getQExact( level ); }else{  cerr << "ERROR: requested custom rule with level " << level << " the tabulated rules end at " << custom->getNumLevels()-1 << endl; exit(1);  }
                default:
                        return level;
        }
}
