...
outs = 1;
prec = 10;
[ lGrid, points ] = tsgMakeGlobal( '_tsgExample4', dim, outs, 'clenshaw-curtis', 'iptotal', prec );
...
disp(['----------------------------------------------------------------------------']);
disp([' Example 4: interpolate f(x,y) = exp( -x^2 ) * cos( y ), using clenshaw-curtis iptotal rule']);
disp(['    using polynomials of total degree up to ',num2str(prec)]);
disp(['    grid has ',num2str(size(points,1)),' points']);
disp(['    interpolant at (0.3,0.7): ',num2str(res,16)]);
disp(['    error: ',num2str(abs(res - exp( -0.3^2 ) * cos( 0.7 )),16 )]);
disp([' ']);
...
disp(['    error           ',num2str(gerr),'       ',num2str(serr),'  seconds']);
...
% 
% 
% disp([' Example 5b: interpolate f(x1,x2,x3,x4) = exp( -x1^2 ) * cos( x2 ) * exp( -x3^2 ) * cos( x4 )']);
% disp(['    comparign the performance of Global and Sequence grids with leja nodes']);
% disp(['    using polynomials of total degree up to ',num2str(prec)]);
% disp(['    grid has ',num2str(size(points,1)),' points']);
% disp(['    both grids are evaluated at 1000 random points']);
...
% 
% tic;
% tsgLoadValues( lGrid, vals * ones( 1, outs ) );
% gstage2 = toc;
% 
% pnts = [ -1 + 2 * rand( 1000, 4 ) ];
% tres = ( exp( -pnts(:,1).^2 ) .* cos( pnts(:,2) ) ) .* ( exp( -pnts(:,3).^2 ) .* cos( pnts(:,4) ) );
...
% disp(['  evaluate       ',num2str(gstage3),'           ',num2str(sstage3),'    seconds']);
% disp(['  error           ',num2str(gerr),'       ',num2str(serr),'  seconds']);
% disp([' The difference between the two approximations is: ',num2str(max(abs( res(:,1) - sres(:,1) )))]);
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% err1 = max( abs( res - tres ) );
% [ res ] = tsgEvaluate( lGrid2, pnts );
% err2 = max( abs( res - tres ) );
% [ res ] = tsgEvaluate( lGrid3, pnts );
% err3 = max( abs( res - tres ) );
