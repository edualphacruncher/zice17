classdef nfxp
    ...
    methods (Static)
        ...
        function [ev, pk, F, iterinfo]=solve(ev, P, cost, mp, options)
            ...
            %             iterinfo.converged (=1 if converged, 0 otherwise)
            %             iterinfo.tol:    [1xniter matrix : error bound for each iteration]
            %             iterinfo.rtol:   [1xniter matrix :relative error bound between two successive approximations]
            %             iterinfo.nkiter: [1xniter matrix : indicator for NK iterations (0/1)]
            %
            ...
            for k=1:opt.max_fxpiter; %poli-algorithm loop (switching between SA and N-K and back)
                ...
                for j=1:opt.max_cstp;
                    ...
                    iterinfo.tol(iter)=tolm1;
                    iterinfo.rtol(iter)=rtolm;
                    iterinfo.nkiter(iter)=0;
                    iter=iter+1;
                ...
                if opt.printfxp>0
                    fprintf('Elapsed time for successive approximations: %3.5f (seconds)\n',toc(solutiontime));
                end
                ...
                if opt.printfxp>0
                    fprintf('Total elapsed time after %d loop(s) in poly-algorithm : %3.5f (seconds)\n',k, toc(solutiontime));
                end

                if iterinfo.converged
                    if opt.printfxp>0
        ...
        function [pp, pp_K, pp_R] = eqb(mp,P, pk)
            ...
            tmp=bsxfun(@times, P(:,2:mp.n), pk(2:mp.n,1)');
            pl=([1-(sum(tmp,2)) tmp]);
            y=0.99999999;
            ...
            for i=1:10000    ;
                ...
                if tol<0.000000001; 
                    fprintf('nfpx.eqb, converged in %d iterations, tol=%g\n', i, tol);
                    break;
                end
            end;
            pp_K=pp;  
            ...
            pp_R=pp.*(1-pk)'; 
        end % endo of NFXP.eqb




                    % This function computes the expected demand for bus engines over a fixed interval of time
            ...
            % for (x,i) as a function of RC, so the user can trace out how the entire distribution of 
