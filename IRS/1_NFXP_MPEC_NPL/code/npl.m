...
classdef npl
    methods (Static) 
        ...
        function [pK]=lambda(Vsigma, mp, P) % Calculates the choice probabilities at the grid points.          
            ...
            VK = - c + mp.beta*P*Vsigma;
            VR = - mp.RC - c(1) + mp.beta*P(1,:)*Vsigma;
            pK = 1./(1+exp(VR-VK));   
        end
        ...
        function [mp, pk, logl, K]=estim(theta0, pk0, data, P, mp, Kmax);  % NPL algorithm for K-PI estimators
            ...
            options =  optimset('Display','off', 'GradObj','on', 'TolFun',1E-6,'TolX',0,'Hessian','on');
            if nargin==5;
                Kmax=100;
            end
            ...
            for K = 1:Kmax
                ...
                Fu = bsxfun(@times, P, pk0)  + bsxfun(@times, P(1,:), 1-pk0);
                Finv=inv(speye(mp.n) - mp.beta*Fu);
